...
# # 2 types de return: pour freq=1, 2 et 3 on a le mois.
# Sinon on a que la semaine
def remplace(L, i, j=None):  # On fait les exercices iàj
...
def repartition_jours(freq, muscle, cardio):
    ...
    if cardio and not muscle:  # cas trivial où on ne fait que du cardio
    ...
    if freq == 4:
        ...
        if cardio:
            L[0] = card
            remplace(L[2], 3, 5)  # on ajoute
        L = L * 4  # bras aux jour 3 avec epaules et abdos
    if freq == 5:
        remplace(L[0], 3, 5)  # on fait bras
        remplace(L[1], 6)  # dos
        remplace(L[2], 7)  # pec
...
def p_opt(obj, T_max=T_max1, Reps=Rep, Exos=Exo):
    def g(p_exo):  # notre fonction d'optimisation
        Xa = np.dot(p_exo, Exos)
        distance = np.linalg.norm(
            Xa / np.linalg.norm(Xa) - obj
        )  # Distance du vecteur normalisé
...
def opt_mois_avec_mem(day_repetition, T_max=T_max1, Reps=Rep, Exos=Exo):
    L = day_repetition
    Exo_mem = np.column_stack(
        [Exos, np.zeros(Exo.shape[0])]
    )  # on creé la colonne mémoire
    e = 1
    a = np.append(L[0], 0)
