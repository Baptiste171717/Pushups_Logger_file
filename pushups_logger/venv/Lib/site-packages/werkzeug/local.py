    ...
    def __init__(self, context_var: ContextVar[list[T]] | None = None) -> None:
        if context_var is None:
            ...
            context_var = ContextVar(
                f"werkzeug.LocalStack<{id(self)}>.storage")
...
class LocalManager:
    ...
    def __init__(
        self,
        locals: None | (Local | LocalStack |
                        t.Iterable[Local | LocalStack]) = None,
    ) -> None:
        if locals is None:
            self.locals = []
...
class LocalProxy(t.Generic[T]):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(local, Local):
            if name is None:
                raise TypeError(
                    "'name' is required when proxying a 'Local' object.")
    ...
    __call__ = _ProxyLookup(lambda self, *args, **
