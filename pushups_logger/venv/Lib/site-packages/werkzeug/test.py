...
def stream_encode_multipart(
    ...
    for key, value in _iter_data(data):
        ...
        if reader is not None:
            ...
            if filename is None:
                write_binary(encoder.send_event(
                    Field(name=key, headers=headers)))
            else:
                write_binary(
                    encoder.send_event(
            ...
            while True:
                ...
                if not chunk:
                    write_binary(encoder.send_event(
                        Data(data=chunk, more_data=False)))
                    break

                write_binary(encoder.send_event(
                    Data(data=chunk, more_data=True)))
        else:
            ...
            write_binary(encoder.send_event(
                Field(name=key, headers=Headers())))
            write_binary(encoder.send_event(
                Data(data=value.encode(), more_data=False)))
...
class EnvironBuilder:
    ...
    def __init__(
        ...
        data: None | (t.IO[bytes] | str | bytes |
...
class Client:
    ...
    def __init__(
        ...
        self.response_wrapper = t.cast(
            t.Type["TestResponse"], response_wrapper)
    ...
    def set_cookie(
        ...
        **kwargs: t.Any,
        ...
        cookie = Cookie._from_response_header(
            domain, "/", dump_cookie(key, value,
                                     domain=domain, path=path, **kwargs)
        )
...
class Cookie:
    ...
    def _from_response_header(cls, server_name: str, path: str, header: str) -> te.Self:
        ...
        return cls(
            ...
            max_age=int(params["max-age"]
                        or 0) if "max-age" in params else None,
